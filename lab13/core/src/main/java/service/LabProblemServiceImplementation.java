...
public class LabProblemServiceImplementation implements LabProblemService {
  ...
  public Optional<LabProblem> addLabProblem(@Valid LabProblem labProblem)
    ...
    log.trace(
        "addLabProblem - method entered: labProblem={}",
            labProblem);
  ...
  public List<LabProblem> getAllLabProblems() {
    ...
    List<LabProblem> result = StreamSupport.stream(repository.findAll().spliterator(),false).collect(Collectors.toList());
  ...
  public Optional<LabProblem> getLabProblemById(Long id) {
    ...
    Optional<LabProblem> labProblem = repository.findById(id);
  ...
  @Override
  @Transactional
  public LabProblem updateLabProblem(@Valid LabProblem labProblem)
      throws ValidatorException {
   ...
   */
  @Override
  public List<LabProblem> filterLabProblemsByProblemNumber
...
//    Iterable<LabProblem> labProblems = repository.findAll((root,query,cb)-> cb.equal(root.get("problemNumber"),problemNumber));
    Iterable<LabProblem> labProblems = repository.findByProblemNumber(problemNumber);
